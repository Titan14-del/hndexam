      ...
      "chapters": [
        ...
        },
        {
          ...
          "content": [
            ...
            { "type": "text", "value": "<b>How to read:</b> The system is split into three tiers: the Client Tier (UI), the Application Tier (API logic and authentication), and the Data Tier (database and cache). The UI depends on the API, and the API depends on the database and cache." },
            { "type": "text", "value": "<hr>" },
            { "type": "text", "value": "<b>3. Deployment Diagram</b> \u2014 Shows the physical hardware (nodes) and which software components run on each node. Nodes are drawn as 3D boxes. This diagram answers: where does each piece of software physically run?" },
            ...
            { "type": "text", "value": "<b>How to read:</b> Three physical machines exist: a Client Machine (runs the browser app), an Application Server (runs Nginx and Node.js), and a Database Server (runs MySQL and backup service). Arrows show network communication between machines." },
            ...
            { "type": "text", "value": "<b>How to read:</b> The process starts at the solid circle. The user enters credentials, then the system validates them. If valid, the dashboard is shown. If invalid, an error appears, and the user can retry up to 3 times before the account is locked. Parallel forks (not shown here) would allow two activities to happen simultaneously — like saving an order AND sending a confirmation email at the same time." }
          ],
          "examTips": ["Know the difference between aggregation (weak whole-part, hollow diamond) and composition (strong lifetime-tied, filled diamond)", "Use Case diagrams are the most common in exams — be able to identify actors and use cases from a scenario", "Sequence diagrams show time order — read from top to bottom", "Activity diagrams use fork bars for parallel activities"]
        },
        {
          "id": "dfd-erd",
          "title": "DFD & ERD",
